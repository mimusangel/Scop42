...
/*   Updated: 2017/11/10 08:46:41 by mgallo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		texture_generate_buffer(t_scop *scop)
{
	...
	while (++i < scop->obj.tcount * 3)
	{
		...
		normal.y /= len;
		normal.z /= len;
		scop->obj.buff[i * 2] = 0.5f + (atan2(normal.z, normal.x)) / (2.0f * SCOP_PI);
		scop->obj.buff[i * 2 + 1] = 0.5f - (asin(normal.y)) / SCOP_PI;
		// printf("u: %f, v: %f\n", scop->obj.buff[i * 2], scop->obj.buff[i * 2 + 1]);
	}
	/*
	** uv.x = 0.5f + (atan2(normal.z, normal.x)) / (2.0f * SCOP_PI);
	** uv.y = 0.5f - (asin(normal.y)) / SCOP_PI;
	*/
}
